 ...
 * Prior to using this interface, an Application is required to obtain a handle to its associated Security Domainís 
...
public interface SecureChannel extends Shareable
{
    ...
    * The returned length of the ìwrappedî data is set to the value of the sLength parameter minus 2 (indicating the status  bytes are no 
    ...
    * @exception <code>java.lang.SecurityException</code><li> if <code>baBuffer</code> is not accessible in the callerís context 
    ...
    * the incoming command will remain as is within the incoming APDU object and the returned length of the ìunwrappedî data is set to the value 
    ...
    * When complying with the Secure Channel Protocol rules, the returned length of the ìunwrappedî data is set to the value of the sLength parameter
