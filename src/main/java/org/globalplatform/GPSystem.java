...
public class GPSystem
{
   ...
   * <code>Applet.getShareableInterfaceObject()</code> method: ëclientAIDí is set to the AID of the
   * current applet context (i.e. the requesting on-card entity) and ëparameterí is set to 
   ...
   * the following parameters: <code>ëGPRegistryEntryí</code> is set to the GPRegistryEntry interface of the current applet
   * context (i.e. the requesting on-card entity), <code>ësServiceNameí</code> first byte is set to <code>FAMILY_CVM</code>
